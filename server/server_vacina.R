  #server para a aba VACINA (03-ago-2021, 23:39h)
  
 #dados base para análise
 dado_vacina <- reactive({dadoi <- serie_vac
                         if(input$radio_vacina == 2){
                         dadoi <- lapply(serie_vac, function(x){x[(nrow(x)-6):nrow(x),]})}
                         if(input$radio_vacina == 3){
                         dadoi <- lapply(serie_vac, function(x){x[(nrow(x)-13):nrow(x),]})}
                         dadoi
                })
                
  dado_fabricante <- reactive({
                         dadoi <- doses_fabricante
                         if(input$radio_vacina == 2){
                         periodo <- seq(max(doses_fabricante[,2])-6, max(doses_fabricante[,2]), by = 'day')
                         dadoi <- doses_fabricante[doses_fabricante[,2] %in% periodo,] }
                         if(input$radio_vacina == 3){
                         periodo <- seq(max(doses_fabricante[,2])-13, max(doses_fabricante[,2]), by = 'day')
                         dadoi <- doses_fabricante[doses_fabricante[,2] %in% periodo,] }
                         dadoi
                })
  ...
  tabela_vacina <- reactive({dadoi <- dado_vacina()[-1]
                            #essa linha não funciona no linux (??)
                            reg_pop$prop_pop <- with(reg_pop, populacao/sum(populacao))
 ...
 output$serie_fab_regiao <- renderHigh({
                        ...
                        datai <- unique(dadoi[,2])
                        dado <- lapply(split(dadoi, dadoi$fabricante), function(x){
                                 fabricante <- x[1,1]
                                 x <- left_join(as.data.frame(datai),x,  by = c('datai' = 'vacina_dataAplicacao')) 
                                 list('name' = fabricante, 'data' = x[,3])
                                 }) %>% unname
                        tipo <- 'line'
